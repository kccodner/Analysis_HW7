...
from forecast_functions import (
    ...
    make_5day_forecast_recent_average,
    load_model,
)
...
parser.add_argument('--model', default='longterm_avg',
                    choices=['longterm_avg', 'recent_5day_avg'])
...
elif args.model == 'recent_5day_avg':
    print("\n--- Step 2: Generate previous 5-day average forecast ---")
    forecast_df = make_5day_forecast_recent_average(recent, args.forecast_date)
    model_label = 'Previous 5-Day Average'
...
ax.plot(
    ...
    color='steelblue',
    linewidth=1.3,
    label='Recent Observed (30 days)'
)
ax.plot(
    forecast_df.index,
    forecast_df['Forecast_cfs'],
    'ro--',
    linewidth=1.5,
    markersize=6,
...
ax.set_title(
    f'Verde River 5-Day Forecast | Starting {forecast_date_ts.date()} ({model_label})'
)
ax.legend()
plt.tight_layout()

os.makedirs('outputs', exist_ok=True)
plot_path = os.path.join('outputs', f'forecast_plot_{args.model}.jpg')
